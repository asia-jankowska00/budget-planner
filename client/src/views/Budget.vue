<template>
  <section
    v-if="budgets.length > 0 && selectedBudget"
    id="budgets"
  ...
  >
    <div class="control">
      ...
      <Icon
        ...
        v-if="selectedBudget.owner.id === user.id"
      />
    </div>

    <div
      id="budgetCollaborators"
      v-if="budgetCollaborators && !isBudgetLoading"
    >
      <p
        class="collaborator"
        v-for="person in budgetCollaborators"
        :key="'collaborator' + person.id"
      >
  ...
  <div v-else-if="budgets.length === 0" class="empty-view">
...
export default {
  ...
  computed: {
    ...
    isBudgetLoading() {
      return (
        this.isBudgetLoadingSources ||
        this.isBudgetLoadingCollaborators ||
        this.isLoadingBudgetTransactions
      );
    },
    budgetTotal: function() {
      let total = 0;
      this.budgetSources.forEach(
        (source) =>
          (total += source.convertedAmount
  ...
  created() {
    if (!this.budgetSources && this.selectedBudget) {
      this.$store
        .dispatch("getBudgetSources", this.selectedBudget.id)
        .catch((err) => {
          M.toast({
            html: err.response.data.message
              ? err.response.data.message
    ...
    if (!this.budgetCollaborators && this.selectedBudget) {
      this.$store
        .dispatch("getBudgetCollaborators", this.selectedBudget.id)
        .catch((err) => {
          M.toast({
            html: err.response.data.message
              ? err.response.data.message
    ...
    if (!this.budgetTransactions && this.selectedBudget) {
      this.$store
        .dispatch("getBudgetTransactions", this.selectedBudget.id)
        .catch((err) => {
          M.toast({
            html: err.response.data.message
              ? err.response.data.message
  ...
  watch: {
    selectedBudget: function(budget) {
      if (budget) {
        this.$store
          .dispatch("getBudgetTransactions", budget.id)
          .catch((err) => {
            M.toast({
              html: err.response.data.message
                ? err.response.data.message
        ...
        this.$store
          .dispatch("getBudgetCollaborators", budget.id)
          .catch((err) => {
            M.toast({
              html: err.response.data.message
                ? err.response.data.message
